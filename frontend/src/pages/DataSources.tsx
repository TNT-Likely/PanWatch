    ...
    return (
      <section key={type} className="card p-4 md:p-6">
        ...
        {(!groupedSources[type] || groupedSources[type].length === 0) ? (
          <p className="text-body text-muted-foreground text-center py-6">暂无{label}数据源</p>
        ) : (
                  ...
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                        ...
                        {source.is_orphan && (
                          <Badge variant="destructive" className="text-mini px-1.5 py-0.5">
                        ...
                        {source.engine_attached && source.health && source.health.success_rate != null && (
                          <span className="flex items-center gap-1 text-mini text-muted-foreground">
                            <span className={`inline-block w-1.5 h-1.5 rounded-full ${
                              ...
                              : source.health.success_rate >= 0.8 ? 'bg-warning' : 'bg-destructive'}`} />
          ...
          <div className="space-y-4 mt-2">
            ...
            {Object.keys(form.config || {}).length > 0 && (
              <details className="text-body-sm">
                ...
                <textarea
                  className="mt-2 w-full font-mono text-caption p-2 border border-border rounded bg-background min-h-[100px]"
        ...
        >
          ...
          <div className="space-y-4 mt-2 pr-1">
            ...
            <div className="flex items-center gap-4 p-3 rounded-lg bg-accent/30">
              ...
              <div className="flex-1">
                <div className="text-caption text-muted-foreground">数据量</div>
                <div className="text-body font-medium">{testResult?.count ?? 0} 条</div>
              </div>
              <div className="flex-1">
                <div className="text-caption text-muted-foreground">耗时</div>
                <div className="text-body font-medium">{testResult?.duration_ms ?? 0} ms</div>
              </div>
            </div>
              ...
              <div>
                ...
                <div className="space-y-1.5 max-h-40 overflow-y-auto">
                  {testResult.logs.map((log, i) => (
                    <div key={i} className="flex items-start gap-2 p-2 rounded-lg bg-accent/30 text-caption">
                  ...
                  {testResult.source_type === 'flash_news' && testResult.items.map((item, i) => {
                    ...
                    return (
                      <div key={i} className="flex items-start gap-2 p-2 rounded-lg bg-accent/30">
                        ...
                        <span className="text-caption text-muted-foreground flex-shrink-0">{flashItem.time}</span>
                      </div>
                    )
                  })}
                    ...
                    return (
                      <div key={i} className="flex items-center justify-between p-2 rounded-lg bg-accent/30">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-caption text-muted-foreground">ROE {fundItem.roe?.toFixed(2) ?? '-'}%</span>
                        </div>
                      </div>
                    )
                  })}
