...
export function ApprovalCard({ approval, onDecision }: ApprovalCardProps) {
  ...
  return (
    <section className="max-w-[85%] rounded-xl border border-amber-500/30 bg-amber-500/5 px-3 py-3 text-body">
      <div className="flex items-start gap-2.5">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            <span className="rounded-full bg-amber-500/10 px-1.5 py-0.5 text-mini text-amber-700 dark:text-amber-300">
              {RISK_LABELS[approval.risk]}
            </span>
          </div>
          <p className="mt-1 text-muted-foreground">{approval.summary}</p>
          {decisionStatus ? (
            <p className={`mt-1.5 text-caption ${decisionStatusClass}`}>
              {decisionStatus}
            </p>
          ) : approval.expires_at && (
          ...
          {failed && <p className="mt-1.5 text-caption text-destructive">提交决定失败，请重试。</p>}
          {approval.status === 'pending' && (
            <div className="mt-3 flex items-center gap-2">
              <button
                ...
                disabled={disabled}
                className="rounded-lg bg-primary px-2.5 py-1.5 text-secondary font-medium text-primary-foreground transition-colors hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-50"
              >
              ...
              <button
                ...
                disabled={disabled}
                className="rounded-lg border border-border bg-background px-2.5 py-1.5 text-secondary font-medium text-muted-foreground transition-colors hover:bg-accent hover:text-foreground disabled:cursor-not-allowed disabled:opacity-50"
              >
