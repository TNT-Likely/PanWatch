...
def _realtime_search(query: str, market: str = "", limit: int = 20) -> list[dict]:
    ...
    for item in items:
        ...
        security_type_name = (item.get("SecurityTypeName") or "").strip()
        code_raw = (item.get("Code") or "").strip().upper()

        # 先归一化代码(基金/股票的 CN 代码为纯 6 位数字)
        code = item.get("Code", "")

        # 场内 ETF/LOF 判定:东财 Classify=Fund,且为 CN 6 位代码、
        # 前缀 5(SH ETF/LOF,含 50xxxx)/15/16(SZ ETF/LOF)。
        # 排除场外基金(0/1 非场内前缀,如 001186)与期权(8 位代码,Classify=Option)。
        cn_fund_symbol = _normalize_symbol(code, "CN")
        is_cn_etf = (
            classify == "Fund"
            and cn_fund_symbol.isdigit()
            and len(cn_fund_symbol) == 6
            and (cn_fund_symbol[0] == "5" or cn_fund_symbol[:2] in ("15", "16"))
        )
        ...
        if is_cn_etf:
            ...
            or any(ch in security_type_name for ch in ("沪", "深", "北"))
            or code_raw.endswith(".BJ")
            or code_raw.startswith("BJ")
        ):
            stock_market = "CN"
            sec_type = "stock"
        elif classify == "HKStock" or "港" in security_type_name:
            stock_market = "HK"
            sec_type = "stock"
        elif classify == "UsStock" or "美" in security_type_name:
            stock_market = "US"
            sec_type = "stock"
        else:
            continue  # 跳过期权/指数/场外基金/债券等

        # 市场筛选
        if market and stock_market != market:
            continue

        # 只保留股票(排除债券等);US ETF 暂不支持(保持原有过滤)
