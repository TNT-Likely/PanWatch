...
_PRICE_ALERT_TARGET_PATTERN = re.compile(r"(?:提醒|价格提醒|price\s*alert|alert)", re.IGNORECASE)
...
def is_write_intent(content: str) -> bool:
    ...
    if not text:
        return False
    if not _WRITE_VERB_PATTERN.search(text) or not _PRICE_ALERT_TARGET_PATTERN.search(text):
        return False
