...
class TestNormalizeAllocations(unittest.TestCase):
    def test_fill_missing_markets(self):
        """歸一化 — 缺失市場補 0 且覆蓋三市場"""
        out = normalize_allocations({"TW": 0.6})
        self.assertEqual(set(out.keys()), set(ALL_MARKETS))
        self.assertAlmostEqual(out["TW"], 0.6)
        self.assertEqual(out["US"], 0.0)

    def test_clamp_negative_to_zero(self):
        """歸一化 — 負比例 clamp 為 0"""
        out = normalize_allocations({"US": -0.2, "TW": 0.5})
        self.assertEqual(out["US"], 0.0)
        self.assertAlmostEqual(out["TW"], 0.5)
    ...
    def test_none_input(self):
        ...
        self.assertEqual(out, {"TW": 0.0, "US": 0.0})
...
class TestAllocationsFromExcluded(unittest.TestCase):
    def test_exclude_us_renormalizes_rest(self):
        ...
        self.assertEqual(out["US"], 0.0)
        self.assertAlmostEqual(out["TW"], 1.0, places=4)
    ...
    def test_all_excluded_fallback_cn(self):
        """遷移 — 全部排除時兜底投 A 股"""
        out = allocations_from_excluded(["TW", "US"])
        self.assertEqual(out, {"TW": 1.0, "US": 0.0})
...
class TestMarketAllocationsOrDefault(unittest.TestCase):
    ...
    def test_configured_is_normalized(self):
        """账户比例 — 已配置则归一化返回"""
        acc = SimpleNamespace(
            market_allocations={"TW": 1.0, "US": 0}, initial_capital=1_000_000
        )
        out = market_allocations_or_default(acc)
        self.assertEqual(out, {"TW": 1.0, "US": 0.0})
    ...
    def test_sum_le_one_invariant(self):
        """账户比例 — 合理配置合计不超过 1"""
        acc = SimpleNamespace(
            market_allocations={"TW": 0.7, "US": 0.3}, initial_capital=1_000_000
        )
        out = market_allocations_or_default(acc)
